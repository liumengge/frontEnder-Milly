...
1. 下载并引入echarts  `dist/echarts.min.js`
2. 准备一个具备大小的DOM容器，注意：这个盒子必须要有宽度和高度
...
- grid：直角坐标系内绘图网格，可以控制折线图/柱状图的大小 

- title：标题组件

- tooltip：图标的提示框组件

- legend：图例组件，如果series里面有name值，可以不用设置legend
...
- 上述内容包含地图模块的制作(主要是写中间的球体)：
> 地图模块高度为810px， 里面包含4个盒子： chart放图表模块， 球体盒子，旋转1， 旋转2
> 球体图片模块 map1  大小为518px 要加背景图片 因为要缩放100%  定位到最中央 透明度0.3
> 旋转1：map2， 大小为643px，要加背景图片，因为要缩放100%， 定位到中央，透明度0.6  做旋转动画
> 旋转：map3 大小为566px，要加背景图片因为要缩放100%  定位到中央  做旋转动画， 注意是逆时针 
...
需求1：修改折线图大小，显示边框设置颜色为`#012f4a`，显示刻度标签
...
需求2：修改图例组件中的颜色为`#4c9bfd`，距离右侧right为10%
...
- 两条线的颜色：`#00f2f1`,`#ed3f35`
- 把折线修改为圆滑曲线：series数据中添加smooth为true
...
series: [
  ...
  },
  {
      ...
      stack: '总量',
      smooth: true,
      data: [120, 111, 101, 234, 290, 130, 310]
  }
]
```
