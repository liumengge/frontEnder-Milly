- [企业级CRM客户管理系统](#企业级crm客户管理系统)
  ...
  - [4. 首页开发](#4-首页开发)
  - [5. 员工管理](#5-员工管理)
    - [5.1 员工列表处理](#51-员工列表处理)
    - [5.2 员工信息的新增与修改](#52-员工信息的新增与修改)
    - [5.3 删除与批量删除](#53-删除与批量删除)
  - [6. 客户管理 - 分页处理](#6-客户管理---分页处理)
...
#### 5. 员工管理
- 如果有组织结构的权限，右侧显示的是员工管理中的员工列表
  - 获取部门信息，绑定到下拉列表框中
  - 获取员工列表信息：需要把用户筛选的信息传递给服务器
  - 以上两个部分的依赖性不大，可以使用并行请求方式来处理
##### 5.1 员工列表处理

- 获取部门信息，将部门信息绑定到下拉框中，默认显示全部
- 获取员工列表数据，将员工信息展示在右侧表格中
- 当选择顶部下拉菜单中的某一个部门时，能够显示对应部门的员工，以传参的形式发起Ajax请求，获取员工信息
- 当在搜索框中输入关键字后按ENTER,下方列表能够显示匹配筛选条件的员工信息(输入关键字后发送ajax请求拿到员工信息)

##### 5.2 员工信息的新增与修改
- 使用同一个页面实现员工信息的新增与修改，如果传了userId就是修改信息，如果没有传递userId就是新增员工信息。
